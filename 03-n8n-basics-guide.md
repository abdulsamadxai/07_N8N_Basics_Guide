...
Notice the tabs at the top - **Editor** (where you build), **Executions** (where every past run gets logged), and **Evaluations**. On the right side there are small icons for adding nodes, searching, and other panel options. Bottom left has zoom controls. We will come back to these as we go.
...
| Webhook | Runs when an outside app sends it a request | A Flask/FastAPI route |
...
This node calls an outside API - exactly like `requests.get()` in Python. The globe icon and "GET:" label tell you what it's set up to do. That red warning triangle means it's not configured yet - no URL has been entered.
...
- **INPUT** (left) - the data coming in from the previous node. Says "No input connected" since nothing feeds into it yet.
- **Parameters** (middle) - the actual settings. For HTTP Request that's Method (GET, POST, etc), URL, Authentication, and toggles for sending Query Parameters, Headers, or a Body.
- **OUTPUT** (right) - what the node returns after it runs. You hit **Execute step** to actually run just this one node and see real output, or use **set mock data** to fake a response so you can keep building downstream nodes without making a real call yet.

### Code Node

![Code node on canvas](images/06-code-node-canvas.png)

This is the node for when you need actual logic - loops, conditions, custom calculations - anything the built-in nodes can't do out of the box. Orange `{}` icon.

![Code node configuration panel](images/07-code-node-config.png)

Same three-column layout again. The important bits:

- **Mode** - "Run Once for All Items" or "Run Once for Each Item." This decides whether your code runs a single time on the whole batch of data, or loops automatically once per item.
- **Language** - JavaScript or Python.
- The code editor itself, pre-filled with a starter example that loops over `$input.all()` and returns the result.

`$input.all()` here does the same job as looping over a list of dictionaries in Python - it's just how n8n hands you the incoming data inside a Code node.

---

## Step 4 - The AI Agent

Now the important part. This is the node your whole chatbot/agent gets built around.

![AI Agent node on canvas](images/08-ai-agent-canvas.png)

Look at the shape of this. The Agent box sits on top, and three things hang off the *bottom* of it through diamond-shaped connectors - **Chat Model**, **Memory**, and **Tool**. That's not a coincidence, that's n8n visually showing you the exact same four pieces from the Agent guide:
...
| (inside the Agent itself) | The system prompt / instructions | Your system message string |

Chat Model has a little red `*` next to it because it's required - an agent can't run without a model. Memory and Tool are both optional. Notice the red warning triangle on the Agent box itself right now - that's because nothing is plugged into Chat Model yet.

Opening the Agent node shows its settings:

![AI Agent configuration panel](images/09-ai-agent-config.png)

- **Source for Prompt (User Message)** - set to "Connected Chat Trigger Node", meaning it automatically grabs whatever the user typed into the chat.
- **Prompt (User Message)** - shows `{{ $json.chatInput }}`. This is an **expression**, n8n's way of pulling data from an earlier node. This one specifically means "grab the `chatInput` field from whatever triggered this workflow" - same job as `user_input = input("You: ")` in Python.
...
- Scroll down and you'll see the same Chat Model / Memory / Tool slots repeated here too, with `+` buttons to attach a node to each one directly from this panel.
...
n8n doesn't lock you into one AI company. Anthropic, Groq, OpenAI - pick whichever one you have an API key for. This is the exact same idea as swapping `ChatOpenAI` for `ChatGroq` in LangChain Python code, except here it's a dropdown instead of an import line.

Picking one (Groq, in this case) asks you for credentials:

![Groq API credentials modal](images/11-groq-credentials.png)

This is important - **n8n doesn't give you an API key**. You have to go to that provider's own website (console.groq.com, platform.openai.com, console.anthropic.com, etc), create an account, generate a key yourself, and paste it in here. Once saved, n8n stores it securely and every node afterward just references it by name ("Groq account 18") - you never have to paste the key again.
...
- **Simple Memory** - remembers the conversation in-memory, no setup needed. This is your `chat_history` list, basically.
- **MongoDB Chat Memory** / **Postgres Chat Memory** - remembers the conversation in a real database, so it survives even if the workflow restarts.
...
A helpful note right at the top says "This node must be connected to an AI agent" - memory nodes don't do anything by themselves, they only work once plugged into an Agent's Memory slot.

- **Session ID** - set to "Connected Chat Trigger Node" by default, so each conversation gets its own separate memory automatically.
- **Table Name** - `n8n_chat_histories`, the actual database table where the conversation gets saved.
- **Context Window Length** - how many past messages get sent back to the model as context. Set to `5` here. Same trade-off as picking a window size for memory in Python - too small and the AI forgets things, too large and you're paying for tokens you don't need.
...
- **Code Tool** - write your own JavaScript function as a tool, same editor as the Code node from Step 3. This is the most direct match to `tool_map` from the Python guide - a function the agent can decide to call.
...
- **MCP Client** - connects to an external MCP server and lets the agent use its tools. If you've touched MCP already, this is the same concept, just wired in visually.

Opening MCP Client shows its settings:

![MCP Client configuration panel](images/15-mcp-client-config.png)

Same "must be connected to an AI agent" note as memory. Key fields are **Endpoint** (the MCP server's URL), **Server Transport** (HTTP Streamable here), **Authentication**, and **Tools to Include** (all tools from that server, or specific ones).

A tool's **description** field (not fully visible in these screenshots, but every tool node has one) is what the agent actually reads to decide *when* to use that tool. No description means the agent doesn't know the tool exists or what it's for - so writing a clear, specific description matters just as much here as writing a good docstring for a Python `@tool` function.
...
You've now seen every piece an AI Agent is built from - trigger, model, memory, and tools - the exact same shape as the chatbot you already built in Python.

**Next up: Building a Basic Chatbot in n8n using the Groq model** - putting a Chat Trigger, an AI Agent, and a Groq Chat Model together into your first real working n8n chatbot, no tools or memory yet, just the core loop working end to end.
